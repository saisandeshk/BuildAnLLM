...
def create_lora_matrices(weight_shape, rank: int, alpha: float, dropout: float = 0.0, weight=None):
    """
    Create LoRA matrices A and B for a given weight shape.

    Args:
        weight_shape: Shape of the base weight (tuple or torch.Size)
        rank: LoRA rank
        alpha: LoRA alpha
        dropout: Dropout rate
        weight: Optional weight tensor to get device/dtype from
    ...
    # Get device and dtype from weight tensor if provided
    ...
    if len(weight_shape) == 2:
        ...
        lora_A = nn.Parameter(torch.empty(
            rank, in_dim, device=device, dtype=dtype))
        lora_B = nn.Parameter(torch.zeros(
            out_dim, rank, device=device, dtype=dtype))
        ...
        lora_A = nn.Parameter(torch.empty(
            n_heads, rank, d_model, device=device, dtype=dtype))
        lora_B = nn.Parameter(torch.zeros(
            n_heads, d_head, rank, device=device, dtype=dtype))
        nn.init.kaiming_uniform_(lora_A, a=math.sqrt(5.0))
    else:
...
def apply_lora_to_attention(attn_module, rank: int = 8, alpha: float = 8.0, dropout: float = 0.0):
    ...
    for param_name in ['W_Q', 'W_K', 'W_V', 'W_O']:
        if hasattr(attn_module, param_name):
            ...
            # Create LoRA matrices (pass weight to ensure correct device/dtype)
            lora_A, lora_B, scaling, dropout_layer = create_lora_matrices(
                weight.shape, rank, alpha, dropout, weight=weight
            )
...
def einsum_with_lora(x, weight, pattern, lora_A, lora_B, scaling, dropout_layer):
    ...
    # This handles edge cases where device mismatch might occur (e.g., MPS issues)
    # Use in-place operations to avoid creating new tensors unnecessarily
    if lora_A.device != x.device:
        lora_A = lora_A.to(x.device)
    if lora_B.device != x.device:
